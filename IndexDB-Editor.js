javascript: (function() {
    if (document.getElementById('indexeddb-editor-root')) return;
    const root = document.createElement('div');
    root.id = 'indexeddb-editor-root';
    document.body.appendChild(root);
    const shadow = root.attachShadow({
        mode: 'open'
    });
    const s = document.createElement('style');
    s.textContent = `#storage-editor-widget{position:fixed;left:50px;top:50px;width:550px;background:#1e1e1e;border-radius:10px;box-shadow:0 10px 40px rgba(0,0,0,0.6);border:1px solid #2d2d2d;z-index:999999;font-family:system-ui,-apple-system,sans-serif;color:#e0e0e0;overflow:hidden}#storage-editor-widget.minimized{width:300px;height:auto}#storage-editor-widget.minimized .ce-body{display:none}.ce-titlebar{background:#252525;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #2d2d2d;cursor:move;user-select:none}.ce-title{font-weight:700;font-size:16px;letter-spacing:0.5px;color:#fff}.ce-controls{display:flex;gap:6px}.ce-btn{width:32px;height:32px;border:none;background:transparent;color:#aaa;border-radius:5px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s}.ce-btn:hover{background:#2d2d2d}.ce-btn.close:hover{background:#dc2626}.ce-body{padding:16px;max-height:450px;overflow-y:auto}.ce-search-bar{display:flex;gap:8px;margin-bottom:14px}.ce-search{flex:1;background:#252525;border:1px solid #2d2d2d;border-radius:6px;padding:10px 12px;color:#e0e0e0;font-size:13px;outline:none;padding-left:36px}.ce-search:focus{border-color:#3d3d3d}.ce-search-wrap{position:relative;flex:1}.ce-search-icon{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:#6b7280;width:16px;height:16px}.ce-action-btn{padding:10px 14px;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;display:flex;align-items:center;gap:6px;transition:all 0.2s;background:#252525;color:#fff;border:1px solid #2d2d2d}.ce-action-btn:hover{background:#2d2d2d}.ce-action-btn.danger{background:#3d1f1f;border-color:#4d2f2f}.ce-action-btn.danger:hover{background:#4d2f2f}.ce-info{color:#9ca3af;font-size:12px;margin-bottom:14px;padding:8px 12px;background:#252525;border-radius:6px;border:1px solid #2d2d2d}.ce-items{}.ce-item{background:#252525;border:1px solid #2d2d2d;border-radius:6px;margin-bottom:8px;overflow:hidden;transition:all 0.2s}.ce-item:hover{border-color:#3d3d3d}.ce-item-header{padding:10px 12px;background:#1e1e1e;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #2d2d2d}.ce-key{color:#fff;font-family:monospace;font-size:13px;font-weight:600;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ce-delete-btn{padding:6px;background:transparent;border:none;color:#888;cursor:pointer;border-radius:4px;display:flex;align-items:center;justify-content:center;transition:all 0.2s}.ce-delete-btn:hover{background:#dc2626;color:#fff}.ce-value{padding:12px;cursor:pointer;transition:background 0.2s}.ce-value:hover{background:#1e1e1e}.ce-value pre{color:#d1d5db;font-family:monospace;font-size:12px;white-space:pre-wrap;word-break:break-all;margin:0;line-height:1.5}.ce-edit-area{padding:12px}.ce-textarea{width:100%;background:#1e1e1e;border:1px solid #2d2d2d;border-radius:6px;padding:10px;color:#e0e0e0;font-family:monospace;font-size:12px;resize:vertical;min-height:100px;outline:none;box-sizing:border-box}.ce-textarea:focus{border-color:#3d3d3d}.ce-edit-controls{display:flex;gap:8px;margin-top:8px}.ce-save-btn{background:#1f3d1f;color:white;border:1px solid:#2f4d2f}.ce-save-btn:hover{background:#2f4d2f}.ce-cancel-btn{background:#252525;color:white;border:1px solid #2d2d2d}.ce-cancel-btn:hover{background:#2d2d2d}.ce-empty{text-align:center;color:#888;padding:40px 0;font-size:13px}.ce-db-selector{margin-bottom:14px}.ce-select{width:100%;background:#252525;border:1px solid #2d2d2d;border-radius:6px;padding:10px 12px;color:#e0e0e0;font-size:13px;outline:none}.ce-select:focus{border-color:#3d3d3d}`;
    shadow.appendChild(s);
    const w = document.createElement('div');
    w.id = 'storage-editor-widget';
    w.innerHTML = `<div class="ce-titlebar" id="ce-titlebar"><div class="ce-title">IndexDB Viewer</div><div class="ce-controls"><button class="ce-btn" id="ce-minimize"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"></line></svg></button><button class="ce-btn close" id="ce-close"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button></div></div><div class="ce-body"><div class="ce-db-selector"><select class="ce-select" id="ce-db-select"><option value="">Select Database...</option></select></div><div class="ce-search-bar"><div class="ce-search-wrap"><svg class="ce-search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.35-4.35"></path></svg><input type="text" class="ce-search" placeholder="Search records..." id="ce-search"></div><button class="ce-action-btn danger" id="ce-clear"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>Clear</button><button class="ce-action-btn" id="ce-refresh"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"></polyline><polyline points="1 20 1 14 7 14"></polyline><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>Refresh</button></div><div class="ce-info" id="ce-info"></div><div class="ce-items" id="ce-items"></div></div>`;
    shadow.appendChild(w);
    const site = window.location.hostname || 'localhost';
    let pos = {
            x: 50,
            y: 50
        },
        drag = false,
        off = {
            x: 0,
            y: 0
        },
        editingKey = null,
        currentDB = null,
        currentStore = null;
    const tb = shadow.getElementById('ce-titlebar');
    tb.addEventListener('mousedown', e => {
        if (e.target.closest('.ce-btn')) return;
        drag = true;
        off = {
            x: e.clientX - pos.x,
            y: e.clientY - pos.y
        }
    });
    document.addEventListener('mousemove', e => {
        if (drag) {
            pos = {
                x: e.clientX - off.x,
                y: e.clientY - off.y
            };
            w.style.left = pos.x + 'px';
            w.style.top = pos.y + 'px'
        }
    });
    document.addEventListener('mouseup', () => drag = false);
    shadow.getElementById('ce-minimize').addEventListener('click', () => w.classList.toggle('minimized'));
    shadow.getElementById('ce-close').addEventListener('click', () => root.remove());
    const loadDatabases = async () => {
        const dbs = await indexedDB.databases();
        const select = shadow.getElementById('ce-db-select');
        select.innerHTML = '<option value="">Select Database...</option>' + dbs.map(db => `<option value="${db.name}">${db.name} (v${db.version})</option>`).join('')
    };
    const getData = async () => {
        if (!currentDB || !currentStore) return [];
        return new Promise((resolve, reject) => {
            const req = indexedDB.open(currentDB);
            req.onsuccess = () => {
                const db = req.result;
                if (!db.objectStoreNames.contains(currentStore)) {
                    resolve([]);
                    return
                }
                const tx = db.transaction(currentStore, 'readonly');
                const store = tx.objectStore(currentStore);
                const getAll = store.getAll();
                getAll.onsuccess = () => resolve(getAll.result);
                getAll.onerror = () => resolve([])
            };
            req.onerror = () => resolve([])
        })
    };
    const render = async (keepEditing = false) => {
        if (!currentDB || !currentStore) {
            shadow.getElementById('ce-items').innerHTML = '<div class="ce-empty">Select a database and object store</div>';
            shadow.getElementById('ce-info').textContent = `0 records • Website: ${site}`;
            return
        }
        const search = shadow.getElementById('ce-search').value.toLowerCase();
        const data = await getData();
        const filtered = data.filter(item => JSON.stringify(item).toLowerCase().includes(search));
        const items = shadow.getElementById('ce-items');
        const info = shadow.getElementById('ce-info');
        const totalSize = new Blob([JSON.stringify(data)]).size;
        info.textContent = `${filtered.length} records • ${(totalSize/1024).toFixed(2)} KB • Website: ${site}`;
        if (filtered.length === 0) {
            items.innerHTML = '<div class="ce-empty">' + (search ? 'No matching records found' : 'No records found') + '</div>';
            editingKey = null;
            return
        }
        items.innerHTML = filtered.map((item, idx) => {
            const key = item.id || item.key || idx;
            const display = JSON.stringify(item, null, 2);
            if (keepEditing && key === editingKey) {
                return `<div class="ce-item" data-key="${key}"><div class="ce-item-header"><span class="ce-key">Record ${key}</span><button class="ce-delete-btn" data-action="delete" data-key="${key}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></div><div class="ce-value"><div class="ce-edit-area"><textarea class="ce-textarea" id="ce-edit-textarea">${display}</textarea><div class="ce-edit-controls"><button class="ce-action-btn ce-save-btn" data-action="save" data-key="${key}">Save</button><button class="ce-action-btn ce-cancel-btn" data-action="cancel">Cancel</button></div></div></div></div>`
            }
            return `<div class="ce-item" data-key="${key}"><div class="ce-item-header"><span class="ce-key">Record ${key}</span><button class="ce-delete-btn" data-action="delete" data-key="${key}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></div><div class="ce-value" data-action="edit" data-key="${key}"><pre>${display.length>300?display.substring(0,300)+'...':display}</pre></div></div>`
        }).join('');
        shadow.getElementById('ce-items').addEventListener('click', async e => {
            const btn = e.target.closest('[data-action]');
            if (!btn) return;
            const action = btn.dataset.action;
            const key = btn.dataset.key;
            if (action === 'delete') {
                if (!confirm(`Delete record ${key}?`)) return;
                const req = indexedDB.open(currentDB);
                req.onsuccess = () => {
                    const db = req.result;
                    const tx = db.transaction(currentStore, 'readwrite');
                    const store = tx.objectStore(currentStore);
                    store.delete(key);
                    tx.oncomplete = () => {
                        editingKey = null;
                        render()
                    }
                }
            } else if (action === 'edit') {
                editingKey = key;
                render(true);
                setTimeout(() => {
                    const textarea = shadow.getElementById('ce-edit-textarea');
                    if (textarea) textarea.focus()
                }, 0)
            } else if (action === 'save') {
                const textarea = shadow.getElementById('ce-edit-textarea');
                if (textarea) {
                    try {
                        const newData = JSON.parse(textarea.value);
                        const req = indexedDB.open(currentDB);
                        req.onsuccess = () => {
                            const db = req.result;
                            const tx = db.transaction(currentStore, 'readwrite');
                            const store = tx.objectStore(currentStore);
                            store.put(newData);
                            tx.oncomplete = () => {
                                editingKey = null;
                                render()
                            }
                        }
                    } catch (err) {
                        alert('Invalid JSON: ' + err.message)
                    }
                }
            } else if (action === 'cancel') {
                editingKey = null;
                render()
            }
        })
    };
    shadow.getElementById('ce-db-select').addEventListener('change', async e => {
        currentDB = e.target.value;
        if (!currentDB) {
            render();
            return
        }
        const req = indexedDB.open(currentDB);
        req.onsuccess = () => {
            const db = req.result;
            const stores = Array.from(db.objectStoreNames);
            if (stores.length > 0) {
                currentStore = stores[0];
                render()
            } else {
                shadow.getElementById('ce-items').innerHTML = '<div class="ce-empty">No object stores found</div>'
            }
        }
    });
    shadow.getElementById('ce-search').addEventListener('input', () => {
        editingKey = null;
        render()
    });
    shadow.getElementById('ce-refresh').addEventListener('click', () => {
        editingKey = null;
        render()
    });
    shadow.getElementById('ce-clear').addEventListener('click', async () => {
        if (!currentDB || !currentStore) return;
        if (!confirm('⚠️ Clear ALL records in this store? This cannot be undone!')) return;
        const req = indexedDB.open(currentDB);
        req.onsuccess = () => {
            const db = req.result;
            const tx = db.transaction(currentStore, 'readwrite');
            const store = tx.objectStore(currentStore);
            store.clear();
            tx.oncomplete = () => {
                editingKey = null;
                render()
            }
        }
    });
    loadDatabases();
    render()
})();
