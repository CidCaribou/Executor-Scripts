javascript: (function() {
    ...
    const getData = () => {
        ...
        document.cookie.split(';').forEach(c => {
            ...
            if (k) data[k] = decodeURIComponent(v || '')
        });
        return data
    };
    const render = (keepEditing = false) => {
        const search = shadow.getElementById('ce-search').value.toLowerCase();
        const data = getData();
        const filtered = Object.entries(data).filter(([k]) => k.toLowerCase().includes(search));
        ...
        info.textContent = `${filtered.length} cookies • ${(totalSize/1024).toFixed(2)} KB • Website: ${site}`;
        if (filtered.length === 0) {
            items.innerHTML = '<div class="ce-empty">' + (search ? 'No matching cookies found' : 'No cookies found') + '</div>';
            editingKey = null;
            return
        }
        items.innerHTML = filtered.map(([k, v]) => {
            if (keepEditing && k === editingKey) {
                return `<div class="ce-item" data-key="${k.replace(/"/g,'&quot;')}"><div class="ce-item-header"><span class="ce-key">${k}</span><button class="ce-delete-btn" data-action="delete" data-key="${k.replace(/"/g,'&quot;')}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></div><div class="ce-value"><div class="ce-edit-area"><textarea class="ce-textarea" id="ce-edit-textarea">${v}</textarea><div class="ce-edit-controls"><button class="ce-action-btn ce-save-btn" data-action="save" data-key="${k.replace(/"/g,'&quot;')}">Save</button><button class="ce-action-btn ce-cancel-btn" data-action="cancel">Cancel</button></div></div></div></div>`
            }
            return `<div class="ce-item" data-key="${k.replace(/"/g,'&quot;')}"><div class="ce-item-header"><span class="ce-key">${k}</span><button class="ce-delete-btn" data-action="delete" data-key="${k.replace(/"/g,'&quot;')}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></div><div class="ce-value" data-action="edit" data-key="${k.replace(/"/g,'&quot;')}"><pre>${v.length>300?v.substring(0,300)+'...':v}</pre></div></div>`
        }).join('');
        shadow.getElementById('ce-items').addEventListener('click', e => {
            const btn = e.target.closest('[data-action]');
            if (!btn) return;
            const action = btn.dataset.action;
            const key = btn.dataset.key;
            if (action === 'delete') {
                if (!confirm(`Delete cookie "${key}"?`)) return;
                ...
                const textarea = shadow.getElementById('ce-edit-textarea');
                if (textarea) {
                    document.cookie = `${key}=${encodeURIComponent(textarea.value)}; path=/;`;
                    const saveBtn = btn;
                    const original = saveBtn.innerHTML;
                    saveBtn.innerHTML = 'Saved!';
                    saveBtn.style.background = '#2f4d2f';
                    setTimeout(() => {
                        saveBtn.innerHTML = original;
                        saveBtn.style.background = '#1f3d1f'
                    }, 1500)
                }
    ...
    shadow.getElementById('ce-clear').addEventListener('click', () => {
        if (!confirm('⚠️ Clear ALL cookies? This cannot be undone!')) return;
        document.cookie.split(';').forEach(c => {
